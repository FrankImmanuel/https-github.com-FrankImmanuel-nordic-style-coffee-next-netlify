...
import { UnauthenticatedError, isUnauthenticatedError, UnexpectedDataError, UnauthorizedError } from "../../errors.js";
...
class PrismicRepositoryManager extends BaseManager {
  ...
  async fetchEnvironments(args) {
    ...
    try {
      ...
      return {
        error: new UnexpectedDataError("Unexpected Error while fetching Environments")
      };
    }
    if (res.ok) {
      const json = await res.json();
      ...
      if (error) {
        return {
          error: new UnexpectedDataError(`Failed to decode environments: ${error.errors.join(", ")}`)
        };
      }
      if ("results" in value) {
        let environments = value.results;
        if (!(args == null ? void 0 : args.includeAllPersonalEnvironments)) {
          try {
